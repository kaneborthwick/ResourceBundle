...
class ResourceHandler extends AbstractHandler {
	...
	: \Psr\Http\Message\ResponseInterface{
		...
		$jsonContent = $this->serialize($resources, $configuration->getSerializationGroups());
	...
	private function serialize($data, $serializationGroups = []) {
		$groups = ['Default'];
		$context = new SerializationContext();
		$context->setSerializeNull(true);
		if (count($serializationGroups) > 0) {
			$groups = array_merge($groups, $serializationGroups);
		}
